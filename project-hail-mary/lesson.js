...
document.querySelector("#save-report").addEventListener("click", () => {
  ...
  if (!popup) return;
  const printReport = popup.print.bind(popup);
  popup.print = () => {};
  ...
  popup.document.head.insertAdjacentHTML("beforeend", `<style>
    ...
    body { max-width: 780px; font-family: Arial, sans-serif; color: #102121; background: #ffffff; }
    .report-header { gap: 15px; padding: 0 0 19px; border-bottom: 1px solid #b9d8d6; }
    .report-header img { width: 62px; height: 62px; border: 0; border-radius: 50%; box-shadow: 0 7px 18px rgba(22,185,180,.25); }
    .eyebrow { color: #087f7b; font-size: 10px; letter-spacing: 1.35px; }
    h1, h2, h3 { font-family: Arial, sans-serif; letter-spacing: -0.45px; }
    h1 { font-size: 31px; } h2 { margin: 30px 0 14px; font-size: 20px; } .student { color: #58706f; }
    .summary { margin: 24px 0 29px; padding: 21px 23px; border: 0; border-radius: 20px; background: linear-gradient(120deg, #cafff7, #d9ff00); box-shadow: 0 10px 24px rgba(8,127,123,.14); }
    .summary strong { font-size: 27px; }.summary p { color: #235654; letter-spacing: .7px; }
    .section-grid { gap: 14px; }.section-card { padding: 15px; border: 1px solid #d4e2e1; border-radius: 15px; background: #f9fffe; }
    .card-heading span { font-size: 22px; }.section-card h3 { font-size: 14px; }.section-card p { color: #5a7372; }.bar { overflow: hidden; border: 0; border-radius: 999px; background: #e3eeed; }.bar span { border-radius: inherit; }
    .writing { margin-top: 29px; padding: 23px; border: 1px solid #cde6e4; border-radius: 19px; background: #f1fffd; }.writing blockquote { border: 0; border-radius: 12px; background: #fff; box-shadow: inset 0 0 0 1px #d9e8e7; }.footer { margin-top: 28px; padding-top: 16px; border-top: 1px solid #d4e2e1; color: #607674; }
    @media print { .summary, .bar span { print-color-adjust: exact; -webkit-print-color-adjust: exact; } }
  <\/style>`);
  popup.document.close();
  popup.print = printReport;
  window.setTimeout(printReport, 400);
});
...
document.querySelector("#share-story").addEventListener("click", async () => {
  ...
  try {
    ...
    context.fillRect(150, 1320, 780, 92);
    context.strokeStyle = "#16b9b4";
    context.lineWidth = 3;
    context.strokeRect(150, 1320, 780, 92);
    context.font = "800 33px Arial, sans-serif";
    context.fillStyle = "#ffffff";
    context.shadowColor = "#000000";
    context.shadowBlur = 12;
    context.shadowOffsetY = 4;
    drawCenteredText(context, `I completed ${progress.completed} of ${progress.total} activities!`, 1366, 730, 44);
